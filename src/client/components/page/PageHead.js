...
const PageHead = ({ title, description, path = '/' }) => {
    const pageTitle = title ? `${title} – ${config.appName}` : `${config.appName} – ${config.appTagline}`
    ...
    const fonts = [['Inter', '300,400,500,700']]
    ...
    return (
        <Head>
            ...
            {fonts.map(([fontName, fontWeights]) => (
                <link
                    key={fontName}
                    rel="stylesheet"
                    href={`https://fonts.googleapis.com/css?family=${`${fontName.replace(/ /g, '+')}${
                        fontWeights === undefined ? `:${fontWeights}` : ''
                    }`}&display=swap`}
                />
            ))}
